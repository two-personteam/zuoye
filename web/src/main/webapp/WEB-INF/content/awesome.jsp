...
<html>
  <head>
	...
	-->
<cc:base type="font-awesome,jquery,layer"></cc:base>
<style type="text/css">
...
.fontawesome-icon-list li:hover {
	background-color: rgba(248, 172, 89, 0.88);

}
.ic-selected,.fontawesome-icon-list .selected:hover{
...
.layui-field-title {
  ...
  <body>
    ...
    $('[class*="css("]').each(function(){
    	...
    	var pattern =new RegExp("css\\((.| )+?\\)","igm");
    	var result = cls.match(pattern);
    	if(result){
    		$.each(result,function(){
    			var str = this.substr(4,this.length-5);
    			var kvs = str.split(";");
    			$.each(kvs,function(){
    				var kv = this.split(":");
    				obj.css(kv[0],kv[1]);
...
$(function(){
	...
	$.each(data,function(){
		...
		$('.icons').append(box);
		box.find('legend').html(info.name);
		$.each(info.icons,function(){
			...
			li.click(function(){
				window.icondata.selected = 'fa '+ic;
				$('.icons li').removeClass('ic-selected');
...
<div class="icons" style="padding: 0;margin:0;">
<div class="icon-group" style="display: none;margin-bottom: 10px;clear: both;top:30px;padding-top: 20px">
	<fieldset class="layui-field-title"><legend>分类</legend></fieldset>
