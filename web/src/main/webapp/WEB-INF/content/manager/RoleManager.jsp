...
<body class="gray-bg">
    ...
    var cc = {
		...
		$('#tb_user').bootstrapTable({
			url: '${path}/manage/roles!json.html', //请求后台的URL（*）
			method: 'get', //请求方式（*）
			...
			columns: [{checkbox: true}, 
				...
				{title: '操作',formatter:function(){
					return '<button type="button" class="btn btn-w-m btn-success">授权管理</button>';
				}}
			]
		});
		...
		$("#toolbar > button:eq(2)").click(function(){
		    ...
		    swal({
		    	...
		    	$.post('${path}/manage/roles!delete.html',data,function(data){
		    		if(data=='success'){
		...
		$("#part_add .submit").click(function(){
	    	...
	    	$.post('${path}/manage/roles!save.html',data,function(data){
	    		layer.close(ly);
	    		if(data=='success'){
	    			table.bootstrapTable("refresh");
	    			layer.msg('数据添加成功');
		...
		panel.find(".submit").click(function(){
	    	...
	    	var ly = layer.msg('正在提交到服务器', {icon: 16,shade: 0.3});
	    	$.post('${path}/manage/roles!edit.html',data,function(data){
	    		layer.close(ly);
	    		if(data=='success'){
	    			table.bootstrapTable("refresh");
	    			layer.msg('数据修改成功');
